...
def get_company_info(stock_code):
    try:
        info_dict = {}
        url = (
            "http://www.stockex.co.tt/controller.php?action=view_stock_charts&StockCode="
            + str(stock_code)
        )
        print(url)
        r = requests.get(url)
...
def get_company_history(stock_code, start_date, end_date):
    ...
    """
    url = (
        "http://www.stockex.co.tt/controller.php?action=view_stock_history&StockCode="
        + str(stock_code)
    )
    ...
    for info in range(0, len(data), 5):
        output_dict[data[info].text.replace("\xa0", "").strip()] = {
            "closing_quote": data[info + 1].text.replace("\xa0", "").strip(),
            "change_dollar": data[info + 2].text.replace("\xa0", "").strip(),
            "change_percent": data[info + 3].text.replace("\xa0", "").strip(),
            "volume_traded": data[info + 4].text.replace("\xa0", "").strip(),
        }
    return output_dict
